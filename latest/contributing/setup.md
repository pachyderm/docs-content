...
(Note, as of 07/11/19 {{% productName %}}is using go modules and recommends cloning the code outside of the $GOPATH, we use the location ~/workspace as an example, but the code can live anywhere)
```s
    cd ~/workspace
...
To run it, simply call `./etc/reset.py` from the {{% productName %}}repo root.
