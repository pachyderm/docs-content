...
# metadata # 
title: Branch
description: Learn about the concept of a branch in {{%productName%}}. 
...
A {{%productName%}} branch is a pointer to a commit that
moves along with new commits as they are submitted. By default,
when you create a repository, {{%productName%}} does not create any branches.
...
are how {{%productName%}} knows which data each pipeline relies on, and which branches and
repos should be included in each commit.

Each branch has a `HEAD` which references the latest commit in the
branch. {{%productName%}} pipelines look at the `HEAD` of the branch
